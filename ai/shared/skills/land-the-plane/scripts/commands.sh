#!/usr/bin/env bash
# Command discovery caching script for land-the-plane workflow
# Usage: commands.sh --list | --cache <source> <category> '<commands_json>' | --clear

set -euo pipefail

# Constants
CACHE_DIR=".tmp"
CACHE_FILE="${CACHE_DIR}/commands-cache.json"
DEFAULT_CACHE='[]'
...
list_commands() {
    ...
    if [[ ${#valid_entries[@]} -eq 0 ]]; then
        if [[ ${#stale_sources[@]} -gt 0 ]]; then
            ...
            echo "  commands.sh --cache <source> <category> '<commands_json>'" >&2
        else
            echo "Error: No valid cached commands found. Cache miss." >&2
            echo "Please discover CI commands and cache them with:" >&2
            echo "  commands.sh --cache <source> <category> '<commands_json>'" >&2
        fi
        exit 1
...
Output:
  ...
  Returns empty array [] if no valid cached commands.
