...
_: prev: let
  ...
  delveForArm64 = prev.writeShellScriptBin "dlv" ''
    exec arch -arm64 ${prev.delve}/bin/dlv "$@"
  '';
  delvePerSystem = if system == "aarch64-darwin" then delveForArm64 else prev.delve;
in {
  wrapped = {
